"""
Training script for MeanPool+MLP DTI baseline architecture.
...
def train_fold(
    ...
    model = MeanPoolDTI(
        ...
        d_model=config_arch.protein_d_model,
    ).to(DEVICE)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train MeanPool+MLP DTI baseline on specified dataset."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    print(f"\nTraining MeanPool+MLP Baseline on {args.dataset}")
    ...
    # Print model info
    model_info = MeanPoolDTI(drug_input_dim=config_data.drug_input_dim).to(DEVICE)
