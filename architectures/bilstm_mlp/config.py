"""
Configuration for BiLSTM+MLP DTI architecture.
...
@dataclass
class BiLSTMMLPConfig:
    """Training and model configuration for BiLSTM+MLP."""
    ...
    # BiLSTM encoder (protein)
    bilstm_d_model: int = 64  # Will be halved for hidden_size (bidirectional)
    ...
    # ReduceLROnPlateau for stability (esp. with LSTM)
    scheduler_type: str = "plateau"
    plateau_factor: float = 0.5
    plateau_patience: int = 3
    plateau_verbose: bool = True
    ...
    results_dir: Path = Path("architectures/bilstm_mlp/results")
    logs_dir: Path = Path("architectures/bilstm_mlp/logs")
    checkpoints_dir: Path = Path("architectures/bilstm_mlp/checkpoints")
...
default_config = BiLSTMMLPConfig()
