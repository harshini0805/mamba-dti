"""
Training script for Mamba+AttentionPool+MLP DTI architecture.
...
def train_fold(
    ...
    model = MambaAttnDTI(
        ...
        d_model=config_arch.mamba_d_model,
    ).to(DEVICE)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Mamba+AttentionPool+MLP DTI model on specified dataset."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    print(f"\nTraining Mamba+AttentionPool+MLP on {args.dataset}")
    ...
    # Print model info
    model_info = MambaAttnDTI(drug_input_dim=config_data.drug_input_dim).to(DEVICE)
