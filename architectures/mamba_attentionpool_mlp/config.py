"""
Configuration for Mamba+AttentionPool+MLP DTI architecture.
...
class MambaAttnConfig:
    """Training and model configuration for Mamba+AttentionPool+MLP."""
    ...
    mamba_d_model: int = 64  # Protein sequence dimension
    mamba_d_state: int = 16
    mamba_d_conv: int = 4
    mamba_expand: int = 2

    # AttentionPool parameters (shared for protein and drug)
    attn_pool_init_gain: float = 0.1  # Small gain keeps attention uniform initially
    ...
    results_dir: Path = Path("architectures/mamba_attentionpool_mlp/results")
    logs_dir: Path = Path("architectures/mamba_attentionpool_mlp/logs")
    checkpoints_dir: Path = Path("architectures/mamba_attentionpool_mlp/checkpoints")
...
default_config = MambaAttnConfig()
